    ...
    <div className="flex items-center justify-between p-4 px-6 md:px-10 bg-white border-b border-gray-200 shadow-sm font-sans">
      <div className="flex items-center gap-3">
        ...
        <div className='relative flex items-center'>
          <div className='absolute w-20 h-12 rounded-full bg-green-100 top-1/2 left-1/2 rotate-160 transform -translate-x-1/2 -translate-y-1/2 z-0 opacity-70'></div> 
      ...
      <div className="flex items-center gap-6 text-sm font-medium text-gray-700">
        ...
        <div className="flex items-center gap-4">
          {currentUser.loggedIn ? (
            <>
              <span className="font-semibold text-teal-700 hidden sm:block">{currentUser.name}</span>
              <Link to="/logout" className="hover:text-teal-600 transition-colors">Logout</Link>
              <Link to="/write" className="w-10 h-10 rounded-full bg-teal-200 flex items-center justify-center text-sm font-semibold text-teal-800 hover:bg-teal-300 transition-colors cursor-pointer">
                Write
              </Link>
            </>
          ) : (
